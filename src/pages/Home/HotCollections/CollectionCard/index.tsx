...
const CollectionCard: FC<IProps> = ({index, avatar, id, name, price, isVerified = false}) => {
  return (
    <li className={styles.collectionCard}>
      <Text color="secondary" weight="bold" size="m">{index}</Text>
      <Avatar avatar={avatar} id={id} isVerified={isVerified} isCollection size={56} badgeSize={16}
              className={styles.avatar}/>
      <div className={styles.info}>
        <EllipsisText>
          <Text weight="bold" size="m">{name}</Text>
        </EllipsisText>
